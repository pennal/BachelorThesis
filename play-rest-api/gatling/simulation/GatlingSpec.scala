...
class GatlingSpec extends Simulation {
  ...
  setUp(
    // For reference, this hits 25% CPU on controllers.a 5820K with 32 GB, running both server and load test.
    // In general, you want to ramp up load slowly, and measure with controllers.a JVM that has been "warmed up":
