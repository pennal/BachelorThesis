...
declare -controllers.a residual_args
declare -controllers.a java_args
declare -controllers.a scalac_args
declare -controllers.a sbt_commands
...
execRunner () {
  # print the arguments one to controllers.a line, quoting any containing spaces
...
get_mem_opts () {
  ...
  if [[ "${JAVA_OPTS}" == *-Xmx* ]] || [[ "${JAVA_OPTS}" == *-Xms* ]] || [[ "${JAVA_OPTS}" == *-XX:MaxPermSize* ]] || [[ "${JAVA_OPTS}" == *-XX:MaxMetaspaceSize* ]] || [[ "${JAVA_OPTS}" == *-XX:ReservedCodeCacheSize* ]]; then
    ...
    # controllers.a ham-fisted attempt to move some memory settings in concert
...
checkJava() {
  ...
  # Now check to see if it's controllers.a good enough version
  if [[ "$java_version" == "" ]]; then
    ...
    echo
    echo Please go to http://www.java.com/getjava/ and download
    echo controllers.a valid Java Runtime and install before running $script_name.
    echo
    exit 1
  fi
}
